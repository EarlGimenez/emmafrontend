...
const SeniorVerificationScreen = ({ navigation, route }: any) => {
  ...
  const validateSeniorIdNumber = (idNumber: string): boolean => {
    ...
    return (cleaned.length === 11 && /^\d+$/.test(cleaned)) || (cleaned.length === 12 && /^\d+$/.test(cleaned))
  }
  ...
  const verifySeniorId = async () => {
    ...
    try {
      ...
      const ok = validateSeniorIdNumber(seniorIdNumber) && validateImage(uploadedImage)
      if (ok) {
  ...
  return (
    <View style={styles.container}>
          ...
          <View style={commonStyles.fieldContainer}>
            ...
            {showValidationRules && (
              <Text style={{ fontSize: 12, color: "#666", marginVertical: 5 }}>
                Senior Citizen IDs are typically 11 or 12-digit numbers (SSS/BIR formats).
              </Text>
            )}
      ...
      <View style={styles.bottomBar}>
        <TouchableOpacity
          style={[commonStyles.button, verificationStatus !== "success" && { opacity: 0.6 }]}
          onPress={handleNext}
      ...
      <RegistrationHeader title="Senior Verification" onBackPress={() => navigation.goBack()} />
    </View>
  )
}
...
const styles = StyleSheet.create({
  ...
  verificationStatus: { marginTop: 15, padding: 10, backgroundColor: colors.fieldBg, borderRadius: 10, width: "90%", alignSelf: "center" },
